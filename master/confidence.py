...
def load_data(test_data_path, preprocess_path, fold, batch_size, num_workers):
    test_data_path = r'D:\wangmansheng\data\LUNA\rowfile\subset'
    crop_size = 32
    black_list = []
    preprocess_path = r'D:\wangmansheng\data\LUNA\cls\crop_v3'
    pix_value, npix = 0, 0
...
def test_module(module, test_loader):
    ...
    for batch_idx, (inputs, targets, feat) in enumerate(test_loader):
        ...
        outputs, logits_aux = module(inputs)
        prediction = 0

        if not isinstance(outputs, tuple):

            confidence, prediction = torch.max(outputs.data.softmax(dim=1), 1)# 將output的data數據，用softmax轉換為概率 dim = 1是每行處理

            print('confidence:')
            print(confidence)

            confidence_two = outputs.data.softmax(dim=1)
            print('confidence_two:')
            print(confidence_two)
            ...
            confidence, prediction = torch.max(outputs[0].data, 1)
    ...
    fnr = 100. * FN.data.item() / (TP.data.item() + FN.data.item())
    F1score = 100. * (TP.data.item() + TP.data.item()) / (
        TP.data.item() + TP.data.item() + FN.data.item() + FP.data.item())

    Gmean = np.sqrt(tpr * tnr)

    print(f'acc:{acc}')
    print('tpr ' + str(tpr) + ' fpr ' + str(fpr))
    print('tnr ' + str(tnr) + ' fnr ' + str(fnr))
...
parser.add_argument('--model_path', type=str,
                    default=r'D:\wangmansheng\wangmansheng_2\code\opensource\test\NAS-Lung-master-DARTS_3D_PC_Fair_ASS_M_10F\Fair-PC-DARTS-master\93.33333333333333_503_model.pt',
                    help='ckpt.t7')
parser.add_argument('--fold', type=int, default=9, help='1-5')
parser.add_argument('--batch_size', type=int, default=1)
parser.add_argument('--num_workers', type=int, default=0)
parser.add_argument('--test_data_path', type=str, default=r'D:\wangmansheng\data\LUNA\rowfile\subset9')
parser.add_argument('--preprocess_path', type=str, default=r'D:\wangmansheng\data\LUNA\cls\crop_v3')
